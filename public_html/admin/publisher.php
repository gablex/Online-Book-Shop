...
$id=  $_REQUEST["edid"];
 if(!$id)
  {
$insrtsql    = "insert into publishermaster  (PUB_NAME ,PUB_DESC) values('$name','$Description')";
$result      = $cmdClass->ExecuteQuery($insrtsql);
header("location:controlsystem.php?sec_id=6&cat=1&msg=Details+Entered+Sucessfully");
exit();
  }
 else
   {
  $updatQuery="UPDATE  publishermaster SET PUB_NAME='$name',PUB_DESC='$Description' WHERE PUB_ID='$id'";
  $result=$cmdClass->ExecuteQuery($updatQuery);
  header("location:controlsystem.php?sec_id=6&cat=2&edit_id=$id&msg=Details+Updated+Sucessfully");
  exit();
...
$disquery   = "SELECT * from publishermaster WHERE PUB_ID='$edit_id'";
$res        = $cmdClass->ExecuteQuery($disquery);
$obj        = $cmdClass->getObject($res);
$ex_name    = $obj->PUB_NAME;
$ex_details = stripslashes($obj->PUB_DESC);
$oFCKeditor1->Value=stripslashes($ex_details);
}
?>
<html>
<head>
...
<form name="frm_exchange" method="post" action="" onSubmit="return validate();">
  <div align="center"><span class="logintext">Publisher Details</span><br>
  </div>
  <table width="82%" border="0" align="center" cellpadding="0" cellspacing="1">
	...
	<tr> 
      ...
      <td width="82%"><input name="name" type="text" class="longtxtbox" value="<?php echo $ex_name ;?>"></td>
    </tr>
    <tr> 
