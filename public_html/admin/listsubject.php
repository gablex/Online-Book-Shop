 ...
 ***********/
 ...
 if(isset($_REQUEST['del_id']))//*******Submission Delete record from a table 
     { 
     $sql ="DELETE FROM subjectmaster WHERE SUB_ID=".$_REQUEST['del_id'];
   ...
   $sql="SELECT * FROM subjectmaster  order by SUB_NAME "; //select record from tha table
   $res = $cmdClass->ExecuteQuery($sql);
   $num = $cmdClass->getNumberRows($res);
   if ($num!=0){
   while($obj = $cmdClass->getObject($res)){
    $myData[] =array($obj->SUB_ID  ,$obj->SUB_NAME);
   }
...
<table width="515" border="0" align="center" cellpadding="0" cellspacing="1" class="tableborder" id="thetable">
  ...
  <tr align="center"> 
    ...
    <td width="58" height="25" align="center"><a href="controlsystem.php?sec_id=7&cat=1&edid=<?php echo $datarray[0];?>"><img src="images/b_edit.png" border="0" alt="Edit"></a></td>
    <td width="79" height="25" align="center"><a href="controlsystem.php?sec_id=7&cat=2&del_id=<?php echo $datarray[0];?>" onClick="return confirm_delete();"><img src="images/b_drop.png" border="0" alt="Delete"></a></td>
  </tr>
  <?php
	$i++;}
	?>
