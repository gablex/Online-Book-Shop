 ...
 ***********/
 ...
 if(isset($_REQUEST['del_id']))//*******Submission Delete record from a table 
     { 
			...
			$delquery = "Delete from publishermaster where PUB_ID ='$del_id'";
			$con = $cmdClass->getConnection();
			$cmdClass->ExecuteQuery($delquery);
			$cmdClass->closeConnection($con);
			header("location:controlsystem.php?sec_id=6&cat=2&msg='Record is Deleted Sucessfully'");
			exit();
   ...
   while($obj = $cmdClass->getObject($res)){
    $myData[] =array($obj->PUB_ID ,$obj->PUB_NAME);
   }
   $cmdClass->closeConnection($con);
   $params = array(
    'itemData' => $myData,
...
<table width="515" border="0" align="center" cellpadding="0" cellspacing="1" class="tableborder" id="thetable">
  ...
  <tr align="center"> 
    ...
    <td width="58" height="25" align="center"><a href="controlsystem.php?sec_id=6&cat=1&edid=<?php echo $datarray[0];?>"><img src="images/b_edit.png" border="0" alt="Edit"></a></td>
    <td width="79" height="25" align="center"><a href="controlsystem.php?sec_id=6&cat=2&del_id=<?php echo $datarray[0];?>" onClick="return confirm_delete();"><img src="images/b_drop.png" border="0" alt="Delete"></a></td>
  </tr>
  <?php
	$i++;}
	?>
