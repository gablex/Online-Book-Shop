...
$con = $cmdClass ->getConnection();
 ...
 if(isset($_REQUEST['del_id']))//*******Submission Delete record from a table 
     { 
			...
			$delquery = "Delete from categorysection where  catsec_id  ='$del_id'";
			$cmdClass->ExecuteQuery($delquery);
			header("location:controlsystem.php?sec_id=3&cat=3&msg=Record is Deleted Sucessfully");
			exit();
     } 
if(isset($_REQUEST["edid"])){
	$edid=$_REQUEST["edid"];
	$selQuery="select * from categorysection where catsec_id=$edid";
	$res=$cmdClass->ExecuteQuery($selQuery);
	$objedit=$cmdClass->getObject($res);
	$cmd_name    = $objedit->catsec_name ;
	$displayorder = $objedit->catsec_order; 
}
...
if(isset($_REQUEST["Submit"])) //form submit database inserted values
  {
   ...
   $displayorder = $_REQUEST['displayorder'];
	 ...
	 {  
		   $upquery   = "UPDATE categorysection SET catsec_name = '$sec',catsec_order='$displayorder' where catsec_id ='$edit_id'";	
		   $cmdClass->ExecuteQuery($upquery);
		   header("location:controlsystem.php?sec_id=3&cat=3");
		   exit();
	  }
	else{  
		$insquery    = "insert into categorysection (catsec_name,catsec_order) values ('$sec','$displayorder')";
   ...
   $sql="SELECT catsec_name,catsec_id,catsec_order FROM categorysection order by catsec_name"; //slect record from tha table
   $res = $cmdClass->ExecuteQuery($sql);
   $num = $cmdClass->getNumberRows($res);
   if ($num!=0){
   while($obj = $cmdClass->getObject($res)){
    $myData[] =array($obj->catsec_id ,$obj->catsec_name,$obj->catsec_order);
   }
...
<form name="addsub_cmdty" method="post" action="" onSubmit="return subcmdtyvalidate();">
  ...
  <table width="616" border="0" align="center" cellpadding="0" cellspacing="1">
    ...
    <tr> 
      <td width="3%" height="34" rowspan="2" class="cpbold">&nbsp;</td>
      <td width="33%" class="labeltxt">Browse Section Name</td>
      <td><input name="section" type="text" class="txtbox" id="section" value="<?php echo $cmd_name ;?>">
      </td>
    </tr>
    <tr>
      <td width="33%" height="32" class="labeltxt">Display Order</td>
      <td><input name="displayorder" type="text" class="txtbox" id="section22" value="<?php echo $displayorder ;?>"></td>
    </tr>
	  ...
	  ?>
        <table width="515" align="center" cellspacing="1" class="tableborder">
          <tr align="center" bgcolor="#999999" class="toptxt"> 
            <td width="60" >SLNo</td>
            <td width="226" height="27" >Browse Section Name</td>
            <td width="96" >Disp.Order</td>
            <td width="48">Edit</td>
	...
	foreach ($page_data as $datarray){
	  ?>
          <tr align="center"> 
            ...
            <td width="226" height="27" align="left" class="<?=$cls?>" >&nbsp;&nbsp;<?php echo $datarray[1]; ?>&nbsp;&nbsp;</td>
            <td width="97" align="center" class="<?=$cls?>" ><?php echo $datarray[2]; ?></td>
            <td width="53" height="25" align="center"><a href="controlsystem.php?sec_id=3&cat=3&edid=<?php echo $datarray[0];?>"><img src="images/b_edit.png" border="0" alt="Edit"></a></td>
            <td width="67" height="25" align="center"><a href="controlsystem.php?sec_id=3&cat=3&del_id=<?php echo $datarray[0];?>" onClick="return confirm_delete();"><img src="images/b_drop.png" border="0" alt="Delete"></a></td>
          </tr>
          <?php
	$i++;}
	?>
          <tr bgcolor="#999999"> 
            <td height="26" colspan="7" align="center" valign="middle" class="normaltxt"> 
