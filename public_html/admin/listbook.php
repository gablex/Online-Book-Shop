...
if(isset($_REQUEST['bookcat'])){
   ...
   $sql="SELECT BK_ID,BK_TITLE ,authermaster.AUTH_NAME ,PUB_NAME FROM bookmaster,publishermaster,authermaster WHERE  bookmaster.BK_PUBLISHER = publishermaster.PUB_ID AND BK_LANSTAT = 1 AND bookmaster.BK_AUTH  = authermaster.AUTH_ID AND BK_CAT =".$_REQUEST['bookcat']." order by BK_CDATE DESC"; //select record from tha table
   $res = $cmdClass->ExecuteQuery($sql);
   $num = $cmdClass->getNumberRows($res);
   if ($num!=0){
   while($obj = $cmdClass->getObject($res)){
    $myData[] =array($obj->BK_ID,$obj->BK_TITLE ,$obj->AUTH_NAME ,$obj->PUB_NAME);
   }
   $cmdClass->closeConnection($con);
   $params = array(
    'itemData' => $myData,
