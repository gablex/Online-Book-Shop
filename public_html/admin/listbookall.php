<?php
 ...
 if(isset($_REQUEST['del_id']))//*******Submission Delete record from a table 
     { 
			...
			header("location:controlsystem.php?sec_id=2&cat=7&msg='Record is Deleted Sucessfully'");
			exit();
     } //*********end of delete record from table
   //****** end of sumbit status is changing to the database
   $con = $cmdClass->getConnection();
   $sql="SELECT BK_ID,BK_TITLE ,AUTH_NAME,PUB_NAME FROM bookmaster,publishermaster,authermaster WHERE bookmaster.BK_PUBLISHER = publishermaster.PUB_ID AND BK_LANSTAT = 1 AND bookmaster.BK_AUTH =authermaster.AUTH_ID order by BK_CDATE DESC"; //select record from tha table
...
<table width="795" border="0" align="center" cellpadding="0" cellspacing="1" class="tableborder" id="thetable">
  ...
  <tr align="center"> 
    ...
    <td width="33" height="19" align="center"><a href="controlsystem.php?sec_id=2&cat=1&edid=<?php echo $datarray[0];?>"><img src="images/b_edit.png" border="0" alt="Edit"></a></td>
    <td width="52" height="19" align="center"><a href="controlsystem.php?sec_id=2&cat=7&del_id=<?php echo $datarray[0];?>" onClick="return confirm_delete();"><img src="images/b_drop.png" border="0" alt="Delete"></a></td>
  </tr>
  <?php
	$i++;}
	?>
