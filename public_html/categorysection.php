...
if(isset($_REQUEST["Submit"])) //form submit database inserted values
  {
   ...
   $sec       = $_REQUEST['section'];
	 ...
	 {  
		   $upquery   = "UPDATE categorysection SET catsec_name = '$sec' where catsec_id ='$edit_id'";	
		   $cmdClass->ExecuteQuery($upquery);
		   header("location:controlsystem.php?sec_id=3&cat=3");
		   exit();
	  }
	else{  
		$insquery    = "insert into categorysection (catsec_name) values ('$sec')";
   ...
   $sql="SELECT catsec_name,catsec_id FROM categorysection order by catsec_name"; //slect record from tha table
   $res = $cmdClass->ExecuteQuery($sql);
   $num = $cmdClass->getNumberRows($res);
   if ($num!=0){
   while($obj = $cmdClass->getObject($res)){
    $myData[] =array($obj->catsec_id ,$obj->catsec_name);
   }
...
<form name="addsub_cmdty" method="post" action="" onSubmit="return subcmdtyvalidate();">
  ...
  <table width="616" border="0" align="center" cellpadding="0" cellspacing="1">
    ...
    <tr align="center">
      <td height="22" colspan="3"><br> <table width="515" align="center" cellspacing="1" class="tableborder">
          <tr align="center" bgcolor="#999999" class="toptxt"> 
            <td width="43" >SLNo</td>
            <td width="233" height="27" >Browse Section Name</td>
            <td width="34">Edit</td>
